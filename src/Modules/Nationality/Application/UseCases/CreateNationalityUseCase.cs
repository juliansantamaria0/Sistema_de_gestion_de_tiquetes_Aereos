namespace Sistema_de_gestion_de_tiquetes_Aereos.Modules.Nationality.Application.UseCases;

using Sistema_de_gestion_de_tiquetes_Aereos.Modules.Nationality.Application.Interfaces;

public sealed class CreateNationalityUseCase
{
    private readonly INationalityService _service;
    public CreateNationalityUseCase(INationalityService service) => _service = service;
    public Task<NationalityDto> ExecuteAsync(CreateNationalityRequest request, CancellationToken ct = default)
        => _service.CreateAsync(request, ct);
}
