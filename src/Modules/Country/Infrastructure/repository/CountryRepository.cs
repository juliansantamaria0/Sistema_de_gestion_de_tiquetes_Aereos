...
public sealed class CountryRepository : ICountryRepository
{
    ...
    public async Task<CountryAggregate?> GetByIdAsync(CountryId id, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IEnumerable<CountryAggregate>> GetAllAsync(CancellationToken cancellationToken = default)
    {
    ...
    public async Task AddAsync(CountryAggregate country, CancellationToken cancellationToken = default)
    {
        // Id lo asigna la BD (AUTOINCREMENT); no se incluye en la entidad
        var entity = new CountryEntity { Name = country.Name };
        await _context.Countries.AddAsync(entity, cancellationToken);
    }

    public async Task UpdateAsync(CountryAggregate country, CancellationToken cancellationToken = default)
    {
        var entity = await _context.Countries
            .FirstOrDefaultAsync(c => c.Id == country.Id.Value, cancellationToken)
            ?? throw new KeyNotFoundException($"CountryEntity with id {country.Id.Value} not found.");
    ...
    public async Task DeleteAsync(CountryId id, CancellationToken cancellationToken = default)
    {
        var entity = await _context.Countries
            .FirstOrDefaultAsync(c => c.Id == id.Value, cancellationToken)
            ?? throw new KeyNotFoundException($"CountryEntity with id {id.Value} not found.");
