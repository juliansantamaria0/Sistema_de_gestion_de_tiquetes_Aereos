...
public sealed class CountryService : ICountryService
{
    ...
    public async Task<CountryDto> CreateAsync(string name, CancellationToken cancellationToken = default)
    {
        var aggregate = await _create.ExecuteAsync(name, cancellationToken);
        return new CountryDto(aggregate.Id.Value, aggregate.Name);
    }
    ...
    public async Task<IEnumerable<CountryDto>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        return list.Select(c => new CountryDto(c.Id.Value, c.Name));
    }

    public async Task<CountryDto?> GetByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        var aggregate = await _getById.ExecuteAsync(id, cancellationToken);
        return aggregate is null ? null : new CountryDto(aggregate.Id.Value, aggregate.Name);
    }
